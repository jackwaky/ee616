...
from client.Domain_MOON_Client import Domain_MOON_Client
...
class Domain_MOON_Server():
    def __init__(self, args, model, train_dataset, test_dataset, user_group_train, user_group_test):
        ...
        domain_list = list(set().union(*self.domain_info.values()))

        # Setting for Domain-MOON
        self.previous_nets_per_domain = {i: copy.deepcopy(model) for i in domain_list}
    ...
    def train(self):
        ...
        for epoch in range(self.args.federated_round):
            ...
            # ratio one group & 1-ratio others -- group A in all round // random others
                ...
                selected_user_indices = select_clients_one(domain_info=self.domain_info, num_selected_client=m,
                                                           domain_numbers=self.domain_numbers, epoch=epoch,
                                                           major_domain=major_domain, rpd=self.args.federated_round,
                                                           major_ratio=self.args.major_domain_ratio)
            elif self.args.selection == 'one_rand':
                selected_user_indices = select_clients_one(domain_info=self.domain_info, num_selected_client=m,
                                                           domain_numbers=self.domain_numbers, epoch=epoch,
                                                           major_domain=None, rpd=1,
                                                           major_ratio=self.args.major_domain_ratio)
            elif self.args.selection == 'one_seq':  # one_round, round=1
                selected_user_indices = select_clients_one(domain_info=self.domain_info, num_selected_client=m,
                                                           domain_numbers=self.domain_numbers, epoch=epoch,
                                                           major_domain=major_domain, rpd=1,
                                                           major_ratio=self.args.major_domain_ratio)
            elif self.args.selection == 'one_round_rand':
                selected_user_indices = select_clients_one(domain_info=self.domain_info, num_selected_client=m,
                                                           domain_numbers=self.domain_numbers, epoch=epoch,
                                                           major_domain=None, rpd=self.args.rounds_per_domain,
                                                           major_ratio=self.args.major_domain_ratio)
            elif self.args.selection == 'one_round_seq':
                selected_user_indices = select_clients_one(domain_info=self.domain_info, num_selected_client=m,
                                                           domain_numbers=self.domain_numbers, epoch=epoch,
                                                           major_domain=major_domain, rpd=self.args.rounds_per_domain,
                                                           major_ratio=self.args.major_domain_ratio)

            print(
                f'In round {epoch + 1}, # of selected clients : {len(selected_user_indices)}, selected clients are {selected_user_indices}')
